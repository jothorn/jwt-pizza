...
* **Built-in tools**: Deno's single binary executable replaces the need for dozens of dev dependencies including npm, prettier, eslint, jest, and others. Everything is configured in a single deno.json file (though deno can still read package.json files from existing projects).
...
* **JavaScript engine**: Safari's JavaScriptCore, which has a faster startup time and better memory efficiency than V8. V8 is probably still a bit faster for long-running tasks though.
